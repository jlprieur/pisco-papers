...
static int extract_orbits_from_OC6_gili(FILE *fp_out, FILE *fp_calib, 
...
*/
extract_orbits_from_OC6_gili(fp_out, fp_calib, OC6_fname, is_master_file,
...
static int extract_orbits_from_OC6_gili(FILE *fp_out, FILE *fp_calib, 
                                   ...
                                   int search_for_all)
{
char in_line[256], discov_name[40], comp_name[40], wds_name[40]; 
char buffer[128], previous_comp_name[40];
...
/* If some orbits are found, copy the full lines to the output list file: */
       line_extraction_from_OC6_catalog_gili(OC6_fname, is_master_file,
                                        discov_name, comp_name,
       ...
       if(found) {
        (*n_found_orbits)++;
         ...
         printf("Candidate orbit found for disc=>%s< comp=>%s<\n", 
               discov_name, comp_name);
       } else if(!search_for_all) { 
         printf("*** WARNING: orbit not found for d=>%s< c=>%s<!\n", 
               discov_name, comp_name);
       }
      } else {

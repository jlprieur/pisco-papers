...
* comments_wanted: 1 if comments are wanted, 0 otherwise
* 
* gili_format (flag set to 1 if no ADS column and no orbit info in input file)
*************************************************************************/
...
astrom_read_measures(fp_in, comments_wanted, obj, &nobj, i_filename, i_date, 
                     ...
                     i_notes, with_wds_data, gili_format);
...
/*    fprintf(fp_out,"%s & %s & %s & %.3f & %s & %d & %.3f & %.3f & %.1f%s & %.1f & & %s %s\\\\\n", 
         ...
         me->notes, q_notes);
*/
    fprintf(fp_out,"%s & %s%s & %.3f & %d & %.3f & %.3f & %.1f%s & %.1f & %s &\\\\\n", 
         wds_name, obj[io].discov_name, obj[io].comp_name,
...
/*** Format for subsequent lines of an object */
  ...
  if(me->rho != NO_DATA) 
/*
    fprintf(fp_out,"\\idem & \\idem & \\idem & %.3f & %s & %d & %.3f & %.3f & %.1f%s & %.1f & & \\\\\n", 
         me->epoch, me->filter, 
         me->eyepiece, me->rho, me->drho, me->theta, qflag, me->dtheta); 
         //, me->notes, q_notes);
*/
...
* gili_format: flag set to 1 if ADS column missing and no orbit info.
*
* OUTPUT:
...
int astrom_read_measures(FILE *fp_in, int comments_wanted, OBJECT *obj, 
                         ...
                         int with_wds_data, int gili_format)
{
char b_in[NMAX], b_data[NMAX];
...
*
* i_notes: column nber with the notes
* gili_format: flag set to 1 if no ADS column and no robit info
**************************************************************************/
int astrom_add_new_object_with_wds_data(char *b_data, OBJECT *obj, int *nobj, 
                                        int i_notes, int gili_format)
{ 
OBJECT *ob;
double WR, WT, WY;
...
*
* INPUT:
*   gili_format : flag set to 1 if no ADS column and no orbit information
*
* OUTPUT:
